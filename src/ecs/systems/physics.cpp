...
void PhysicsSystem::ResolveCollision(const CollisionEvent& collision_event) {
    ...
    j /= 1 / collider_a.mass + 1 / collider_b.mass;
    ...
    world::entity_manager->GetComponent<Transform2D>(entity_a)->position += correction * b_inv_mass;
    world::entity_manager->GetComponent<Transform2D>(entity_b)->position -= correction * a_inv_mass;
}
